...
def show_movie(id):
    data = {"id":id,
            "users_id": session['users_id']}
    user_data = {"id":session['users_id']}
    return render_template('movie.html', movie = Movie.get_one_movie_with_user(data), user = User.user_id(user_data), users = User.get_all(), reviews = Review.get_review_with_user(data))
